...
monitor_width = root.winfo_screenwidth()//3
    ...
    def inference_loop(self):
        while True:
            ...
            if inf_stride == buf_stride:
                ...
                #     buffer=input_tensor
                # )
                # analysis(input_data=numpy_frame)
...
def run_pipeline(inf_callback, render_callback, src_size,
    ...
    global final_pipeline,monitor_height,monitor_width
    ...
    PIPELINE = 'v4l2src device=%s ! {src_caps} ! tee name=save' % videosrc
    # PIPELINE = 'filesrc location=./test_data/pt_test.mp4'
    ...
    PIPELINE += """ save. ! decodebin ! videoflip video-direction={direction} ! tee name=t
            t. ! {leaky_q} ! videoconvert ! freezer name=freezer ! rsvgoverlay name=overlay ! videoconvert ! autovideosink
            t. ! {leaky_q} ! videoconvert ! videoscale ! {scale_caps} ! videobox name=box autocrop=true  
                ! {sink_caps} ! {sink_element}
            save. ! decodebin ! videoflip video-direction={direction} ! {leaky_q} ! videoconvert ! videoscale ! video/x-raw,width={width},height={height} ! autovideosink  
            """
    ...
    if is_letter:
        print("편지써라!")
        PIPELINE += """
            save. ! queue ! videoconvert ! mkv. autoaudiosrc ! queue ! audioconvert ! mkv. matroskamux name=mkv ! filesink location=letter.mkv sync=false
            """
    ...
    SINK_CAPS = 'video/x-raw,format=BGR,width={width},height={height}'
    ...
    pipeline = PIPELINE.format(src_caps=src_caps, sink_caps=sink_caps,
        sink_element=SINK_ELEMENT, direction=direction, leaky_q=LEAKY_Q, scale_caps=scale_caps, width=monitor_width, height = monitor_height)
    print('Gstreamer pipeline: ', pipeline)

    final_pipeline = GstPipeline(pipeline, inf_callback, render_callback, src_size)

    final_pipeline.run()

def stop_pipeline():    
    Gtk.main_quit()
    print("STOP!!!!!STOP!!!STOOOOOPP!!!!!!!!!!")
