...
def run(inf_callback, render_callback):
    ...
    parser.add_argument('--videosrc', help='Which video source to use', default='/dev/video2')
...
def test_fps():
    ...
    print(res)
    threading.Timer(1, test_fps).start()
...
def main():
    ...
    def render_overlay(engine, output, src_size, inference_box):
        nonlocal n, sum_process_time, sum_inference_time, fps_counter, gap, cnt, before_nose, body
        ...
        for pose in outputs:
            ...
            cnt+=1
            # print("===================",cnt,int(next(fps_counter)))
            if cnt>=int(next(fps_counter) * 0.5):
                ...
                for label, keypoint in pose.keypoints.items():
                    if label.name == 'NOSE' and keypoint.score > 0:
                        NOSE_Y = keypoint.point[1]
                    elif (label.name == 'LEFT_SHOULDER' and keypoint.score > 0) :
                        LEFT_SHOULDER = keypoint.point[1]
                    elif (label.name == 'LEFT_HIP'):
                        LEFT_HIP = keypoint.point[1]
                ...
                if LEFT_HIP < 480:
                    ...
                    ## body 최소값 필요 : 대안1) 양쪽 어깨 사이의 거리 * 1.5의 크기로 초기화
                print("body : ",body,"=============================Hip : ", LEFT_HIP)
                ## 현재 왼쪽 어깨/엉덩이가 기준인데 해당 포인트가 없는 경우 처리
